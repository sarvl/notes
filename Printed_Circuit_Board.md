#electronics 




## Design
 - Always aim at SMD for cheap manufacture.
 - Stick to common components
 - Get reels or tubes
 - Ensure you are within manufacture capabilities
 - Figure out a way to program board once everything is soldered
 - Use spreadsheet
 - Panelize with single design
 - Add enough support so boards don't bend
 - Add fiducials, especially for high density devices like BGA
 - High density pins should use gold because it can be made really flat
 - Pull back copper from the edges
 - Watch out for number of different components
 - Add connectors for testing
 - When using planes for power/ground, avoid directly routing tracks unless (or)
	 - from decoupling capacitor to power pin
	 - very noise sensitive signal (measured over eg shunt resistor) sent to differential amplifier, also keep ground track short
	 - very large currents
	 - power supplies switching currents

## Videos
- [EEVblog 127 - PCB Design For Manufacture Tutorial - Part 1](https://www.youtube.com/watch?v=VXE_dh38HjU)
- [EEVblog 239 - PCB Design For Manufacture - Part 2](https://www.youtube.com/watch?v=Uemr8xaxcw0)
- [EEVblog 552 - DFM Automated PCB Panel Testing](https://www.youtube.com/watch?v=2zGisPMNstI)

#video_eevblog 